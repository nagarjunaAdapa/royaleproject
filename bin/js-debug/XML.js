...
XML = function(xml) {
  ...
  if (xml != null) {
    ...
    if (xmlStr.indexOf("<") == -1) {
      this.XML__nodeKind = "text";
      this.XML__value = xmlStr;
    } else {
...
XML.insertAttribute = function(att, parent) {
  ...
  parent.XML_addChildInternal(xml);
  return xml;
};
...
var nodeType = nativeNode.nodeType;
    ...
    if (child)
      xml.XML_addChildInternal(child);
  }
};
...
XML.prototype.XML_parseXMLStr = function(xml) {
  ...
  for (var /** @type {number} */ i = 0; i < childCount; i++) {
    ...
    if (node.nodeType == 1) {
      ...
      this.XML__name = XML.getQName(node.localName, node.prefix, node.namespaceURI, false);
...
XML.prototype.XML_getNamespaces = function() {
  if (!this.XML__namespaces)
    this.XML__namespaces = [];
  return this.XML__namespaces;
};
...
XML.prototype.addChild = function(child) {
  if (!child)
    return;
  this.XML_addChildInternal(child);
  this.normalize();
};
 ...
 * @param {XML} child
 */
XML.prototype.XML_addChildInternal = function(child) {
...
XML.prototype.XML_getChildren = function() {
  if (!this.XML__children)
    this.XML__children = [];
  return this.XML__children;
};
...
XML.prototype.XML_getAttributes = function() {
  if (!this.XML__attributes)
    this.XML__attributes = [];
  return this.XML__attributes;
};
...
XML.prototype.addNamespace = function(ns) {
  var /** @type {number} */ i = 0;
  if (this.XML__nodeKind == "text" || this.XML__nodeKind == "comment" || this.XML__nodeKind == "processing-instruction" || this.XML__nodeKind == "attribute")
    return this;
  if (ns.prefix === null)
    return this;
  ...
  var /** @type {number} */ len = this.XML_attributeLength();
  for (i = 0; i < len; i++) {
    if (this.XML__attributes[i].name().prefix == ns.prefix)
      this.XML__attributes[i].name().prefix = null;
  }
  return this;
};
 ...
 * @type {number}
 */
var last = (this.XML_childrenLength()) >>> 0;
    
/**
 * @const
 * @type {XML}
 */
var lastChild = last ? this.XML__children[last - 1] : null;
...
XML.prototype.attribute = function(attributeName) {
  ...
  var /** @type {number} */ len = this.XML_attributeLength();
  for (i = 0; i < len; i++) {
    if (this.XML__attributes[i].name().matches(attributeName))
      list.append(this.XML__attributes[i]);
  }
...
XML.prototype.child = function(propertyName) {
  ...
  if (propertyName.isAttribute) {
    len = this.XML_attributeLength();
    for (i = 0; i < len; i++) {
      if (propertyName.matches(this.XML__attributes[i].name()))
        list.append(this.XML__attributes[i]);
    }
  } else {
    len = this.XML_childrenLength();
    var /** @type {boolean} */ all = propertyName.localName == "*";
    for (i = 0; i < len; i++) {
      if (all || propertyName.matches(this.XML__children[i].name()))
        list.append(this.XML__children[i]);
    }
  }
  list.targetObject = this;
...
XML.prototype.children = function() {
  ...
  //var /** @type {number} */ i = 0;
  var /** @type {XMLList} */ list = new XMLList();
  var /** @type {number} */ len = this.XML_childrenLength();
  for (i = 0; i < len; i++) {
    list.append(this.XML__children[i]);
  }
  list.targetObject = this;
...
XML.prototype.comments = function() {
  ...
  var /** @type {number} */ len = this.XML_childrenLength();
  for (i = 0; i < len; i++) {
    if (this.XML__children[i].nodeKind() == "comment")
      list.append(this.XML__children[i]);
  }
  list.targetObject = this;
  return list;
...
XML.prototype.copy = function() {
  ...
  //var /** @type {number} */ i = 0;
  var /** @type {XML} */ xml = new XML();
  xml.setNodeKind(this.XML__nodeKind);
  xml.setName(this.name());
  xml.setValue(this.XML__value);
  //var /** @type {number} */ len = 0;
  len = this.XML_namespaceLength();
  for (i = 0; i < len; i++) {
    xml.addNamespace(new Namespace(this.XML__namespaces[i]));
  }
  len = this.XML_attributeLength();
  for (i = 0; i < len; i++)
    xml.XML_addChildInternal(this.XML__attributes[i].copy());
  len = this.XML_childrenLength();
  for (i = 0; i < len; i++)
    xml.XML_addChildInternal(this.XML__children[i].copy());
  return xml;
};
...
XML.prototype.XML_deleteChildAt = function(idx) {
  var /** @type {XML} */ child = this.XML__children[idx];
  child.XML__parent = null;
  this.XML__children.splice(idx, 1);
};
...
XML.prototype.descendants = function(name) {
  ...
  if (name.isAttribute) {
    len = this.XML_attributeLength();
    for (i = 0; i < len; i++) {
      if (name.matches(this.XML__attributes[i].name()))
        list.append(this.XML__attributes[i]);
    }
  }
  len = this.XML_childrenLength();
  for (i = 0; i < len; i++) {
    if (this.XML__children[i].nodeKind() == "element") {
      if (name.matches(this.XML__children[i].name()))
        list.append(this.XML__children[i]);
      list.concat(this.XML__children[i].descendants(name));
    }
  }
  return list;
};
...
XML.prototype.elements = function(name) {
  ...
  var /** @type {number} */ len = this.XML_childrenLength();
  for (i = 0; i < len; i++) {
    if (this.XML__children[i].nodeKind() == "element" && name.matches(this.XML__children[i].name()))
      list.append(this.XML__children[i]);
  }
  list.targetObject = this;
...
XML.prototype.equals = function(xml) {
  ...
  if (!org.apache.royale.utils.Language.is(xml, XML))
    return false;
  if (xml.nodeKind() != this.XML__nodeKind)
    return false;
...
XML.prototype.hasAttribute = function(nameOrXML, value) {
  ...
  for (i = 0; i < len; i++) {
    if (name.matches(this.XML__attributes[i].name())) {
      if (!value)
        return true;
      return value == this.XML__attributes[i].getValue();
    }
  }
  return false;
};
...
XML.prototype.getAttributeArray = function() {
  return this.XML__attributes ? this.XML__attributes : [];
};
...
XML.prototype.getChildrenArray = function() {
  return this.XML__children ? this.XML__children : [];
};
...
XML.prototype.getIndexOf = function(elem) {
  return (this.XML__children ? this.XML__children.indexOf(elem) : -1) >> 0;
};
 ...
 * @return {number}
 */
XML.prototype.XML_childrenLength = function() {
  return (this.XML__children ? this.XML__children.length : 0) >> 0;
};
 ...
 * @return {number}
 */
XML.prototype.XML_attributeLength = function() {
  return (this.XML__attributes ? this.XML__attributes.length : 0) >> 0;
};
 ...
 * @return {number}
 */
XML.prototype.XML_namespaceLength = function() {
  return (this.XML__namespaces ? this.XML__namespaces.length : 0) >> 0;
};
...
XML.prototype.XML_getURI = function(prefix) {
  ...
  if (!this.XML__namespaces)
    return "";
  var /** @type {Array} */ namespaces = this.XML_getAncestorNamespaces(this.XML__namespaces);
...
XML.prototype.hasComplexContent = function() {
  var /** @type {number} */ i = 0;
  if (this.XML__nodeKind == "attribute" || this.XML__nodeKind == "comment" || this.XML__nodeKind == "processing-instruction" || this.XML__nodeKind == "text")
    return false;
  //var /** @type {number} */ i = 0;
  var /** @type {number} */ len = this.XML_childrenLength();
  for (i = 0; i < len; i++) {
...
XML.prototype.hasOwnProperty = function(p) {
  ...
  if (name.isAttribute) {
    len = this.XML_attributeLength();
    for (i = 0; i < len; i++) {
      if (this.XML__attributes[i].name().matches(name))
        return true;
    }
  } else {
    len = this.XML_childrenLength();
    for (i = 0; i < len; i++) {
      if (this.XML__children[i].nodeKind() != "element")
        continue;
      if (this.XML__children[i].name().matches(name))
        return true;
    }
  }
  return false;
...
XML.prototype.hasSimpleContent = function() {
  var /** @type {number} */ i = 0;
  if (this.XML__nodeKind == "comment" || this.XML__nodeKind == "processing-instruction")
    return false;
  //var /** @type {number} */ i = 0;
  var /** @type {number} */ len = this.XML_childrenLength();
  for (i = 0; i < len; i++) {
...
XML.prototype.inScopeNamespaces = function() {
  return this.XML__namespaces ? this.XML__namespaces.slice() : [];
};
...
XML.prototype.localName = function() {
  return org.apache.royale.utils.Language.string(this.XML__name ? this.XML__name.localName : null);
};
...
XML.prototype.namespace = function(prefix) {
  ...
  //var /** @type {number} */ i = 0;
  if (prefix) {
    var /** @type {number} */ len = this.XML_namespaceLength();
    for (i = 0; i < len; i++) {
      if (this.XML__namespaces[i].prefix == prefix)
        return this.XML__namespaces[i];
    }
    if (this.XML__parent)
      return this.XML__parent.namespace(prefix);
    return null;
  }
  if (this.XML__nodeKind == "text" || this.XML__nodeKind == "comment" || this.XML__nodeKind == "processing-instruction")
    return null;
...
XML.prototype.namespaceDeclarations = function() {
  ...
  if (this.XML__nodeKind == "text" || this.XML__nodeKind == "comment" || this.XML__nodeKind == "processing-instruction" || this.XML__nodeKind == "attribute")
    return retVal;
  var /** @type {Array} */ declaredNS = this.XML__namespaces ? this.XML__namespaces.slice() : [];
  var /** @type {XML} */ parent = this.XML__parent;
  while (parent) {
 ...
 * @type {string}
 */
XML.prototype.XML__nodeKind = "element";
...
XML.prototype.normalize = function() {
  var /** @type {number} */ len = (this.XML_childrenLength() - 1) >> 0;
  var /** @type {XML} */ lastChild;
  for (var /** @type {number} */ i = len; i >= 0; i--) {
    var /** @type {XML} */ child = this.XML__children[i];
...
XML.prototype.processingInstructions = function(name) {
  ...
  var /** @type {number} */ len = this.XML_childrenLength();
  for (i = 0; i < len; i++) {
    if (this.XML__children[i].nodeKind() == "processing-instruction")
      list.append(this.XML__children[i]);
  }
  list.targetObject = this;
  return list;
...
XML.prototype.removeChild = function(child) {
  ...
  if (child.nodeKind() == "attribute") {
    var /** @type {number} */ len = this.XML_attributeLength();
    for (i = 0; i < len; i++) {
      if (child.equals(this.XML__attributes[i])) {
        removed = this.XML__attributes[i];
        removed.XML__parent = null;
        this.XML__attributes.splice(i, 1);
        return true;
      }
    }
    return false;
  ...
  if (idx < 0)
    return false;
  removed = this.XML__children.splice(idx, 1);
  child.XML__parent = null;
  return true;
};
...
XML.prototype.XML_removeChildByName = function(name) {
  ...
  if (name.isAttribute) {
    len = (this.XML_attributeLength() - 1) >> 0;
    for (i = len; i >= 0; i--) {
      if (this.XML__attributes[i].name().matches(name)) {
        child = this.XML__attributes[i];
        child.XML__parent = null;
        this.XML__attributes.splice(i, 1);
        removedItem = true;
      }
  ...
  len = (this.XML_childrenLength() - 1) >> 0;
  for (i = len; i >= 0; i--) {
    if (this.XML__children[i].nodeKind() != "element") {
      continue;
    }
    if (this.XML__children[i].name().matches(name)) {
      child = this.XML__children[i];
      child.XML__parent = null;
      this.XML__children.splice(i, 1);
      removedItem = true;
    }
  }
  this.normalize();
...
XML.prototype.removeNamespace = function(ns) {
  ...
  if (!org.apache.royale.utils.Language.is(ns, Namespace))
    ns = new Namespace(ns);
  if (ns == this.name().getNamespace(this.XML__namespaces))
    return this;
  len = this.XML_attributeLength();
  for (i = 0; i < len; i++) {
    if (ns == this.XML__attributes[i].name().getNamespace(this.XML__namespaces))
      return this;
  }
  len = this.XML_namespaceLength();
  for (i = (len - 1) >> 0; i >= 0; i--) {
    if (this.XML__namespaces[i].uri == ns.uri && this.XML__namespaces[i].prefix == ns.prefix)
      this.XML__namespaces.splice(i, 1);
    else if (ns.prefix == null && this.XML__namespaces[i].uri == ns.uri)
      this.XML__namespaces.splice(i, 1);
  }
  len = this.XML_childrenLength();
  for (i = 0; i < len; i++) {
    if (this.XML__children[i].nodeKind() == "element")
      this.XML__children[i].removeNamespace(ns);
  }
  return this;
};
...
XML.prototype.replaceChildAt = function(idx, v) {
  ...
  if (org.apache.royale.utils.Language.is(v, XML) && v.nodeKind() != "attribute") {
    if (v.nodeKind() == "element" && (v == this || this.XML_isAncestor(v)))
      throw new TypeError("cannot assign parent xml as child");
    v.setParent(this);
    if (this.XML__children[idx])
      this.removeChild(this.XML__children[idx]);
    this.XML_insertChildAt(v, idx);
  } else if (org.apache.royale.utils.Language.is(v, XMLList)) {
    if (this.XML__children[idx])
      this.XML__children[idx]._parent = null;
    len = (v.length()) >> 0;
    v[0].setParent(this);
    this.XML__children[idx] = v[0];
...
XML.prototype.setAttribute = function(attr, value) {
  ...
  if (org.apache.royale.utils.Language.is(attr, XML)) {
    if (attr.nodeKind() == "attribute") {
      var /** @type {number} */ len = this.XML_attributeLength();
      for (i = 0; i < len; i++) {
        if (this.XML__attributes[i].name().equals(attr.name())) {
          this.XML__attributes[i].setValue(value);
          return value;
        }
      }
      if (value)
  ...
  if (attr.indexOf("xmlns") == 0) {
    ...
    var /** @type {QName} */ qname = this.XML_toAttributeName(attr);
    var /** @type {XML} */ attrXML = new XML();
    attrXML.setNodeKind("attribute");
    attrXML.setName(this.XML_toAttributeName(attr));
    attrXML.setValue(value);
    len = this.XML_attributeLength();
    for (i = 0; i < len; i++) {
      if (this.XML__attributes[i].name().equals(attrXML.name())) {
        this.XML__attributes[i].setValue(value);
        return value;
      }
    }
...
XML.prototype.setLocalName = function(name) {
  if (!this.XML__name)
    this.XML__name = new QName();
  this.XML__name = XML.getQName(name, org.apache.royale.utils.Language.string(this.XML__name.prefix), org.apache.royale.utils.Language.string(this.XML__name.uri), !!(this.XML__name.isAttribute));
};
...
XML.prototype.setName = function(name) {
  if (this.XML__nodeKind == 'text' || this.XML__nodeKind == 'comment')
    return;
  ...
  this.XML__name = XML.getQName(nameRef.localName, nameRef.prefix, nameRef.uri, nameRef.isAttribute);
};
...
XML.prototype.setNamespace = function(ns) {
  var /** @type {string} */ kind = this.XML__nodeKind;
...
XML.prototype.setNodeKind = function(value) {
  if (this.XML__nodeKind != value)
    this.XML__nodeKind = value;
};
...
XML.prototype.text = function() {
  ...
  var /** @type {number} */ len = this.XML_childrenLength();
  for (i = 0; i < len; i++) {
    if (this.XML__children[i].nodeKind() == "text")
      list.append(this.XML__children[i]);
  }
  list.targetObject = this;
  return list;
...
XML.prototype.toString = function() {
  var /** @type {number} */ i = 0;
  //var /** @type {number} */ i = 0;
  if (this.XML__nodeKind == "attribute")
    return this.XML__value;
  if (this.XML__nodeKind == "text")
    return org.apache.royale.utils.Language.string(this.XML__value && this.XML__value.indexOf('<![CDATA[') == 0 ? this.XML__value.substring(9, this.XML__value.length - 3) : this.XML__value);
  if (this.XML__nodeKind == "comment")
    return "";
  if (this.XML__nodeKind == "processing-instruction")
    return "";
  if (this.hasSimpleContent()) {
    var /** @type {string} */ s = "";
    var /** @type {number} */ len = this.XML_childrenLength();
    for (i = 0; i < len; i++) {
      if (this.XML__children[i].nodeKind() == "comment" || this.XML__children[i].nodeKind() == "processing-instruction")
        continue;
      s = s + this.XML__children[i].toString();
    }
    return s;
  }
...
XML.prototype.XML_toXMLName = function(name) {
  ...
  if (org.apache.royale.utils.Language.is(!name, QName)) {
    ...
    if (name.indexOf(":") >= 0) {
      ...
      qname.uri = this.XML_getURI(qname.prefix);
    } else {
 ...
 * @type {string}
 */
var nodeType = this.nodeKind();
  if (nodeType == "text") {
    if (XML.prettyPrinting) {
      var /** @type {string} */ v = XML.trimXMLWhitespace(this.XML__value);
      if (v.indexOf('<![CDATA[') == 0) {
        return indent + v;
      }
      return indent + XML.escapeElementValue(v);
    }
    if (this.XML__value.indexOf('<![CDATA[') == 0)
      return this.XML__value;
    return XML.escapeElementValue(this.XML__value);
  }
  if (nodeType == "attribute")
    return indent + XML.escapeAttributeValue(this.XML__value);
  if (nodeType == "comment")
    return indent + "<!--" + this.XML__value + "-->";
  if (nodeType == "processing-instruction")
    return indent + "<?" + this.name().localName + " " + this.XML__value + "?>";
  if (!ancestors)
    ancestors = [];
  var /** @type {Array} */ declarations = [];
  len = this.XML_namespaceLength();
  for (i = 0; i < len; i++) {
    if (!XML.namespaceInArray(this.XML__namespaces[i], ancestors))
      declarations.push(new Namespace(this.XML__namespaces[i]));
  }
  len = this.XML_attributeLength();
  for (i = 0; i < len; i++) {
    ns = new Namespace(this.XML__attributes[i].name().getNamespace(ancestors.concat(declarations)));
    if (ns.prefix === null) {
      ns.prefix = "";
  ...
  for (i = 0; i < len; i++) {
    strArr.push(" ");
    var /** @type {QName} */ aName = this.XML__attributes[i].name();
    ...
    strArr.push('="');
    strArr.push(XML.escapeAttributeValue(org.apache.royale.utils.Language.string(this.XML__attributes[i].getValue())));
    strArr.push('"');
  }
  ...
  strArr.push(">");
  var /** @type {boolean} */ indentChildren = len > 1 || (len == 1 && this.XML__children[0].nodeKind() != "text");
  //var /** @type {number} */ nextIndentLevel = 0;
  ...
  for (i = 0; i < len; i++) {
    if (XML.prettyPrinting && indentChildren)
      strArr.push("\n");
    strArr.push(this.XML__children[i].toXMLString(nextIndentLevel, ancestors.concat(declarations)));
  }
  if (XML.prettyPrinting && indentChildren) {
    strArr.push("\n");
    if (indentLevel > 0)
  ...
  return {
    ...
    methods: function () {
      return {
        ...
        'toFixed': { type: 'Number', declaredBy: 'XML', parameters: function () { return [ { index: 1, type: 'int', optional: true } ]; }},
 ...
 * @type {number}
 */
XML.prototype.ROYALE_REFLECTION_INFO.compileFlags = 10;
